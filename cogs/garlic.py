...
import textwrap
from utils.configManager import GarlicConfig, BotConfig
...
class Garlic(commands.Cog):
    ...
    @commands.command(usage = '[dark | light] <text to include in post>', aliases=['garlic'])
    ...
    async def makePost(self, ctx, imageFile, words, colour):
        ...
        while font.getsize_multiline(txt)[1] < 0.2*font.getsize_multiline(txt)[0] and fontsize < 80:
            fontsize += 1
            font = ImageFont.truetype(self.config.font, fontsize)
        ...
        draw.text((left, top), txt, colour, font=font, spacing = 50) # put the text on the image
