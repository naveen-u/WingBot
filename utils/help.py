...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        helpMessage = discord.Embed(
            title = 'WingBot Help\t|\tModules',
            color = discord.Color.dark_blue()
        )
        for i in mapping.keys():
            if i is None:
                continue
            helpMessage.add_field(name = i.qualified_name, value = i.description, inline = False)
        helpMessage.set_footer(text = f'Type `{self.botConfig.commandPrefix}help <module>` to learn more about that module.')
    ...
    async def send_cog_help(self, cog):
        ...
        for i in cog.walk_commands():
            if i not in l:
                options += f'`{self.botConfig.commandPrefix}{i.qualified_name}` | {i.help}\n'
                l.append(i)
        helpMessage = discord.Embed(
        ...
        helpMessage.set_footer(text = f'Type `{self.botConfig.commandPrefix}help <command>` to learn more about a command.')
        await self.get_destination().send(embed = helpMessage)
    ...
    async def send_command_help(self, command):
        ...
        helpMessage.add_field(name = 'Usage', value = f'`{self.botConfig.commandPrefix}{command.qualified_name} {command.signature}`', inline = False)
        if command.aliases:
            aliasString = ''
            for i in command.aliases:
                aliasString += f'`{self.botConfig.commandPrefix}{command.full_parent_name} {i}`\n'
